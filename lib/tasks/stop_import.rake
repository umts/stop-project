...
namespace :bus_stops do
  ...
  task :import, [:csv_file] => :environment do |_, args|
    ...
    CSV.foreach(args[:csv_file], headers: true, col_sep: ';') do |row|
      attrs = {}
      ImportTask::STRING_COLUMN_ROW_NAMES.each do |attribute, column_names|
        column_names.each_with_index do |column_name, index|
          if row[column_name] == '1'
            attrs[attribute] = ImportTask::STRING_COLUMN_OPTIONS[attribute][index]
            break
          end
        end
      end
      ImportTask::COLUMN_NAMES.each do |db_column_name, csv_column_name|
